...
int main(){
  ...
  file<<"Xronos eisagwghs eggrafwn sto dyadiko dendro: "<<duration.count()<<" microseconds."<<endl;
  ...
  cin>>answer;
    ...
    auto stop=high_resolution_clock::now();
    auto duration = duration_cast<microseconds>(stop - start);
    file<<"Xronos anazhths se dyadiko dendro: "<<duration.count()<<" microseconds."<<endl;
    for(int i=0;i<100;i++){
...
Bnode* SearchB(Bnode* root,int data) {
	if(root == NULL) {
		cout<<"O arithmos pou anazhthte den yparxei."<<endl;
	}
